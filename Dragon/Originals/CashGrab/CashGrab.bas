10 DIMH(16),A(1),B(1),C(1),P(1),R(1),M(16,11),N(11),X,Y,V,W,D,J,O,R,L,S,M,Q,HP,VP,CL
...
220 ON M(X,Y) GOSUB300,330,360
230 RETURN
240 REM DRAW SCREEN UPDATES
250 R=RND(11):IF N(R)>0 THENN(R)=N(R)-1:O=N(R):M(O,R)=1:PUT(H(O),H(R))-(H(O)+15,H(R)+15),R,PSET
260 IFRND(10)>1THENRETURN
270 O=2+RND(14):R=1+RND(9):IF H(O)>0 AND M(O,R)=0 THEN M(O,R)=2:PUT(H(O),H(R))-(H(O)+15,H(R)+15),C,PSET
280 RETURN
290 REM BURN
300 SCREEN1,0:PLAY"T255AEAEAAAAEEA":SCREEN1,1:M=M-1:IF M THENGOSUB640ELSEGOTO320
310 GOSUB570:GOSUB400
320 RETURN
330 REM SCORE
340 M(X,Y)=0:S=S+10+X:GOSUB520:PLAY"T255BCD":GOSUB270
350 RETURN
...
870 HP=85:VP=180:O$="VERSION":GOSUB1080
880 HP=145:VP=180:N$="0":GOSUB1110
890 HP=157:VP=180:N$="1":GOSUB1110:PSET(153,188,1):PLAY"T255AO3AO2GG"
