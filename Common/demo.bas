1 DIM BX,BY,BH,BW,BC$,T$,TT$,BR,LX,LC$,LZ,LY,LL,TL,LO$
2 REM LIB DIM ABOVE

90 CLEAR 500
100 CLS0:TT$="BASIC LIBRARY DEMO":TBY$="DAVY MITCHELL":TD$="MAY 2023":TV$="VERSION 0.1":TC$=CHR$(175):GOSUB 9010
110 GOSUB 9930
...
1330 LC$=CHR$(255-D*16):LL=13-D:LX=32-(1+D*2):LY=2:GOSUB 9210
1340 NEXT
1399 GOSUB 9900
...
1510 TT$="PROGRAM NAME HERE":TBY$="A COCO CAT":TD$="DATE":TV$="VERSION 1.1":TC$=CHR$(175):GOSUB 9010
1540 GOSUB 9900
...
9003 REM TD$ = DATE
9004 REM TV$ = VERSION
9010 BH=7:BW=LEN(TT$)+2:BY=3:BX=(30-BW)/2+1:BC$=CHR$(143):GOSUB9510
9020 BH=9:BW=LEN(TT$)+4:BY=1:BX=(32-BW)/2:BC$=TC$:GOSUB9410
9030 ?@BX+(BY+1)*32 + 2,TT$;
9040 ?@BX+(BY+3)*32 + ((BW-LEN(TBY$))/2),TBY$;
9050 ?@BX+(BY+5)*32 + ((BW-LEN(TD$))/2),TD$;
9060 ?@BX+(BY+7)*32 + ((BW-LEN(TV$))/2),TV$;
9100 RETURN
...
9410 BR$=""
9420 REM MC10: FOR BR=1TOBW:BR$=BR$+BC$:NEXT
9420 REM COCO/DRAGON: BR$=STRING$(BW,BC$)
...
9910 ?@483,"PRESS ANY KEY TO CONTINUE";:GOSUB9800:RETURN

9920 REM INVERTED ANY KEY PROMPT AT BOTTOM OF SCREEN.
9930 BC$=CHR$(128):?@483,"press"+BC$+"any"+BC$+"key"+BC$+"to"+BC$+"continue";:GOSUB9800:RETURN
