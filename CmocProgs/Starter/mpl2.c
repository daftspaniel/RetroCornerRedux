...
void drawBorder(byte fill)
{
    ...
    memset(SCREEN_ORIGIN, fill, SCREEN_WIDTH);
    memset(SCREEN_ORIGIN + 479, fill, SCREEN_WIDTH);
...
void drawRect(byte fill, int x, int y, int w, int h)
{
    ...
    memset(ptr, fill, w);
    memset((ptr - 1) + ((h - 1) * SCREEN_WIDTH), fill, w);
...
void drawRectFilled(byte fill, int x, int y, int w, int h)
{
    ...
    for (int j = 0; j < h; j++)
    {
        memset(ptr + (j * SCREEN_WIDTH), fill, w);
    }
}
